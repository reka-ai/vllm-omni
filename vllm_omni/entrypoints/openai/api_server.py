...
async def create_video(
    ...
    """
    # Single multipart input_reference (Wan2.2/LTX2 + first-frame Marey via legacy form).
    input_reference_bytes: bytes | None = None
    if input_reference is not None and getattr(input_reference, "filename", None):
        input_reference_bytes = await input_reference.read()
    ...
    # frame_conditions JSON dict mirrors moonvalley_ai's CLI schema:
    # '{"<frame_idx>": {"image_path": "/path/to/image.png"}, ...}'. When set, it
    ...
    if frame_conditions is not None and frame_conditions.strip() != "":
        if input_reference_bytes is not None or parsed_image_reference is not None:
            raise HTTPException(
                status_code=HTTPStatus.BAD_REQUEST.value,
                detail=(
                    "Provide either frame_conditions or input_reference/image_reference, not both."
                ),
            )
    ...
    if frame_conditions is not None and frame_conditions.strip() != "":
        ...
        try:
            single_image = await decode_input_reference(
