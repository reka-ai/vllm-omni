...
class WLAMModelArgs:
    ...
    num_diffusion_extra_layers: int = 0
    ...
    mrope_freq_mode: str = "standard"
    use_dual_attn_weights: bool = True
    use_dual_mlp_weights: bool = True
    use_time_modulation: bool = True
    time_embed_mode: str = "adaLN"
    metadata_embed_mode: str = "in_context"
    use_aspect_ratio_cond: bool = False
    diffusion_head_adaLN: bool = True
    ...
    def from_hf_config(cls, config: Any) -> "WLAMModelArgs":
        ...
        return cls(
            ...
            diffusion_intermediate_size=(
                int(getattr(config, "diffusion_ffn_hidden_size"))
                if getattr(config, "diffusion_ffn_hidden_size", None) is not None
                else None
            ),
            num_hidden_layers=int(
                getattr(text_config, "num_hidden_layers", getattr(config, "num_hidden_layers", getattr(config, "num_layers", 16)))
            ),
            num_diffusion_extra_layers=int(getattr(config, "num_diffusion_extra_layers", 0)),
            ...
            mrope_freq_mode=str(getattr(config, "mrope_freq_mode", "standard")).split(".")[-1].lower(),
            ...
            use_time_modulation=(
                str(getattr(config, "time_embed_mode", "adaLN")).split(".")[-1] == "adaLN"
                if hasattr(config, "time_embed_mode")
                else bool(getattr(config, "use_time_modulation", True))
            ),
            time_embed_mode=str(getattr(config, "time_embed_mode", "adaLN")).split(".")[-1],
            metadata_embed_mode=str(getattr(config, "metadata_embed_mode", "in_context")).split(".")[-1],
            use_aspect_ratio_cond=bool(getattr(config, "use_aspect_ratio_cond", False)),
            diffusion_head_adaLN=bool(getattr(config, "diffusion_head_adaLN", True)),
            expand_instance_eager=bool(getattr(config, "expand_instance_eager", False)),
        )
