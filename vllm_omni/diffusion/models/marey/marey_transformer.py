...
class MareyFluxBlock(nn.Module):
    ...
    def forward(
        ...
        """I2V extensions (off when ``t0_x`` is None):

        - ``x_t_mask`` shape ``[B, T*S, 1]`` of bool. True = noise frame (modulate
          with ``t_x``), False = conditioning frame (modulate with ``t0_x``).
        - ``t0_x`` same shape as ``t_x`` (``[B, hidden]``) — timestep=0 embedding
          used to keep cond-frame tokens unmodulated by the current diffusion
          timestep. Mirrors mv ``dit3d.py`` ``modulate/gate`` calls.

        The ``y`` (text) stream never contains cond frames, so its modulation
        path is unchanged.
        """
        if self.share_weights:
...
class MareyTransformer(nn.Module):
    ...
    def forward(
        ...
        if cond_frames is not None:
            ...
            # I2V parity dump. Dump the POST-``t_block`` value (what flows
            # into blocks as the t0 modulation source) under the name
            # ``t0_emb`` so it matches mv's variable in
            # ``dit3d.mix_time_and_embeddings`` — first return value of
            # ``(t_mlp, t)`` is named ``t0_emb`` on the mv side. Same
            # logical quantity, different local name.
            _dump_pipe = getattr(self, "_dump_pipeline", None)
            if _dump_pipe is not None:
                _dump_pipe._dump_i2v_intermediate("t0_emb", t0_mlp)
        ...
        x = x.reshape(B, T * S, -1)
        # I2V parity dump: AFTER the (B, T+Tf, S, C) -> (B, (T+Tf)*S, C)
        # reshape so the layout matches mv's ``split_sp_variables`` capture
        # (mv reshapes at ``dit3d.py:1079`` before building sp_vars). Both
        # sides are pre-SP-shard (full tensor on every rank).
        if cond_frames is not None:
            _dump_pipe = getattr(self, "_dump_pipeline", None)
            if _dump_pipe is not None:
                _dump_pipe._dump_i2v_intermediate("x_after_concat", x)
                _dump_pipe._dump_i2v_intermediate(
        ...
        # ``dit3d.py:1128-1132``.
        if Tf > 0:
            # I2V parity dump: post-final-layer post-all-gather, pre-slice.
            # Flatten (B, T, S_full, C_out) -> (B, T*S_full, C_out) so the
            # layout matches mv's ``gather_sequence`` return value
            # (dit3d.py:1126).
            _dump_pipe = getattr(self, "_dump_pipeline", None)
            if _dump_pipe is not None:
                _dump_pipe._dump_i2v_intermediate(
                    "x_pre_slice", x.reshape(x.shape[0], T * S_full, -1)
                )
            T = T - Tf
            x = x[:, :T]
