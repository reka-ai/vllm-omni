...
class MareyPipeline(nn.Module, ProgressBarMixin):
    ...
    def forward(self, req: OmniDiffusionRequest) -> DiffusionOutput:
        ...
        if cond_images is not None and cond_frame_indices:
            self.vae.to(device)
            # I2V parity dump: capture the pre-VAE conditioning image stack.
            if hasattr(self, "_dump_cond_images"):
                self._dump_cond_images(cond_images)
            cond_frames, cond_offsets = self._encode_cond_frames(
                cond_images, cond_frame_indices, device=device, dtype=dtype,
            )
            self.vae.to("cpu")
            torch.cuda.empty_cache()
            # I2V parity dump + optional reference injection. Dump first so
            # the recorded value is what vllm-omni computed; load second so
            # downstream uses the reference value when env vars are set.
            if hasattr(self, "_dump_cond_frames"):
                self._dump_cond_frames(cond_frames, cond_offsets)
            if hasattr(self, "_maybe_load_cond_frames"):
                cond_frames = self._maybe_load_cond_frames(cond_frames)
            if hasattr(self, "_maybe_load_cond_offsets"):
                cond_offsets = self._maybe_load_cond_offsets(cond_offsets)
            logger.info(
        ...
        with self.progress_bar(total=len(timesteps)) as pbar:
            for i, t in enumerate(timesteps):
                ...
                # L0 hook: optionally overwrite the local ``z`` with mv's
                # value at this iteration's boundary. No-op outside debug
                # mode. With this active, both the transformer call AND the
                # inline DDPM update operate on mv's state, eliminating
                # cross-step accumulation through the scheduler.
                if hasattr(self, "_maybe_load_pipeline_latents"):
                    z = self._maybe_load_pipeline_latents(z, step_idx=i)
